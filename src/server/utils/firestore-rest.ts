...
export class FirestoreClient {
    ...
    async createDocument(collectionPath: string, documentId: string, data: any): Promise<any> {
        const token = await this.getToken();
    ...
    async patchDocument(path: string, rawData: any, updateMask?: string[]): Promise<any> {
        const token = await this.getToken();
    ...
    async runQuery(parentPath: string, structuredQuery: any): Promise<any> {
        const token = await this.getToken();
    ...
    private fromFirestoreValue(val: any): any {
        ...
        if ('arrayValue' in val) {
            return (val.arrayValue.values || []).map((v: any) => this.fromFirestoreValue(v));
        }
        if ('mapValue' in val) {
