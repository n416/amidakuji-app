...
participants.delete('/events/:eventId/doodle', async (c) => {
  try {
    ...
    if (!memberId || !token) return c.json({ error: '必要な情報が不足しています。' }, 400);
    ...
    const memberDoc = await db.getDocument(`groups/${groupId}/members/${memberId}`);
    if (!memberDoc || !memberDoc.name) return c.json({ error: '認証に失敗しました。', errorCode: 'INVALID_TOKEN' }, 403);
    const memberData = db.firestoreToJson(memberDoc);
    if (memberData.deleteToken !== token) return c.json({ error: '認証に失敗しました。', errorCode: 'INVALID_TOKEN' }, 403);

    if (eventData.status !== 'pending') return c.json({ error: 'このイベントは既に開始されているか終了しています。' }, 403);

    const doodles = eventData.doodles || [];
    const updatedDoodles = doodles.filter((d: any) => d.memberId !== memberId);
...
participants.post('/events/:eventId/join', async (c) => {
  try {
    ...
    if (!name || name.trim() === '') return c.json({ error: '名前は必須です。' }, 400);
    ...
    if (memberData) {
      ...
      const existingColors = (allMembersRes || []).filter((r:any) => r.document).map((r:any) => db.firestoreToJson(r.document).color);
    ...
    if (eventData.status !== 'started') {
      const alreadyJoined = (eventData.participants || []).some((p: any) => p.memberId === finalMemberId);
      if (alreadyJoined) {
        return c.json({
      ...
      const availableSlotIndex = (eventData.participants || []).findIndex((p: any) => p.name === null);
...
participants.post('/events/:eventId/join-slot', async (c) => {
  try {
    ...
    if (!token) return c.json({ error: '認証トークンが必要です。' }, 401);
    if (slot === undefined || slot === null) return c.json({ error: '参加枠が指定されていません。' }, 400);
    ...
    const eventData = db.firestoreToJson(eventDoc);
    if (eventData.status === 'started') return c.json({ error: 'このイベントは既に開始されているため、参加できません。' }, 403);

    const groupId = eventData.groupId;
    const memberDoc = await db.getDocument(`groups/${groupId}/members/${memberId}`);
    if (!memberDoc || !memberDoc.name) return c.json({ error: '認証情報が無効です。', errorCode: 'INVALID_TOKEN' }, 403);

    const memberData = db.firestoreToJson(memberDoc);
    if (memberData.deleteToken !== token) return c.json({ error: '認証情報が無効です。', errorCode: 'INVALID_TOKEN' }, 403);
...
participants.post('/events/:eventId/verify-password', async (c) => {
  try {
    ...
    if (!memberId || !password) return c.json({ error: 'IDと合言葉は必須です。' }, 400);
    ...
    const memberDoc = await db.getDocument(`groups/${groupId}/members/${memberId}`);
    if (!memberDoc || !memberDoc.name) return c.json({ error: 'ユーザーが見つかりません。' }, 401);
...
participants.delete('/events/:eventId/participants', async (c) => {
  try {
    ...
    if (!deleteToken) return c.json({ error: '削除トークンが必要です。' }, 400);
    ...
    const participantIdx = newParticipants.findIndex((p: any) => p.memberId === memberIdToDelete);
    if (participantIdx === -1) {
    ...
    let updatePayload: any = { participants: newParticipants };
...
participants.post('/events/:eventId/acknowledge-result', async (c) => {
  try {
    ...
    if (!token || !memberId) return c.json({ error: '認証情報が不足しています。' }, 400);
    ...
    const participantIndex = (eventData.participants || []).findIndex((p: any) => p.memberId === memberId);
    if (participantIndex === -1) return c.json({ error: 'イベントに参加していません。' }, 404);
...
participants.post('/events/:eventId/fill-slots', requireAuth, async (c) => {
  try {
    ...
    if (!assignments || !Array.isArray(assignments)) return c.json({ error: '割り当てるメンバーのリストが必要です。' }, 400);
    ...
    const emptySlotIndices = updatedParticipants.map((p: any, index: number) => (p.name === null ? index : -1)).filter(index => index !== -1);
