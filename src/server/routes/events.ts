...
events.post('/events', requireAuth, async (c) => {
  try {
    ...
    if (!groupId) return c.json({ error: 'Group ID is required' }, 400);
    if (participantCount < 2) return c.json({ error: 'Prizes must be 2 or more' }, 400);
    ...
    const newEvent = {
      ...
      prizes: prizes.map((p: any) => (typeof p === 'string' ? {name: p, imageUrl: null, rank: 'uncommon'} : {...p, rank: p.rank || 'uncommon'})),
...
events.get('/events/:eventId', async (c) => {
  try {
    ...
    if (!doc || !doc.name) return c.json({ error: 'Event not found' }, 404);
...
events.get('/events/:eventId/public', async (c) => {
  try {
    ...
    const memberId = c.req.header('x-member-id');
    const myParticipant = memberId ? publicParticipants.find((p: any) => p.memberId === memberId) : null;

    let prizeSummaryArray: any[] | null = null;
    ...
    if (eventData.status !== 'started') {
      ...
      const prizeSummary = publicPrizes.reduce((acc: any, prize: any) => {
      ...
      publicPrizes = publicPrizes.map((p: any) => ({ ...p, name: '？？？', imageUrl: null, rank: 'common' }));

      if (myParticipant) {
        publicLines = eventData.lines || [];
        publicParticipants = publicParticipants.map((p: any) => {
        ...
        publicParticipants = publicParticipants.map((p: any) => ({ ...p, name: null, iconUrl: null }));
      }
    } else {
      publicLines = eventData.lines || [];
    }
...
events.put('/events/:eventId', requireAuth, async (c) => {
  try {
    ...
    const eventData = db.firestoreToJson(doc);
    if (eventData.ownerId !== user.targetUserId) return c.json({ error: 'Permission denied' }, 403);

    if (eventData.status === 'started') return c.json({ error: 'Cannot edit started event' }, 400);

    let updateData: any = {
      eventName: eventName || '無題のイベント',
      prizes: prizes.map((p: any) => ({name: p.name, imageUrl: p.imageUrl || null, rank: p.rank || 'uncommon'})),
      participantCount,
      displayPrizeName: !!displayPrizeName,
      displayPrizeCount: !!displayPrizeCount,
...
events.post('/events/:eventId/start', requireAuth, async (c) => {
  try {
    ...
    if (eventData.status === 'started') return c.json({ error: 'Event already started' }, 400);
...
events.post('/events/:eventId/generate-upload-url', requireAuth, async (c) => {
  try {
    ...
    const groupDoc = await db.getDocument(`groups/${groupId}`);
    if (!groupDoc || !groupDoc.name) return c.json({ error: 'Group not found' }, 404);

    const groupData = db.firestoreToJson(groupDoc);
    if (groupData.ownerId !== user.targetUserId) return c.json({ error: 'Permission denied' }, 403);
